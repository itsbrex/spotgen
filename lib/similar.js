...
Similar.prototype.createQueue = function () {
	const artists = this.artists.map(
		(artist) => new Top(this.spotify, this.entry, artist.id, this.limit),
	);
	let queue = new Queue(artists);
	queue = queue.slice(0, this.artistLimit);
	return queue.dispatch().then((result) => result.interleave());
};
...
Similar.prototype.dispatch = function () {
	return this.searchArtists()
		.then(() => this.getArtistRelatedArtists())
