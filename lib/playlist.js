...
function Playlist(spotify, entry, owner, id, limit) {
	...
	this.uri =
		this.owner.id && this.id
			? `spotify:user:${this.owner.id}:playlist:${this.id}`
			: this.uri;
}
...
Playlist.prototype.clone = function (response) {
	for (const prop in response) {
		if (response.hasOwnProperty(prop) && prop !== "limit") {
...
Playlist.prototype.getPlaylist = function (owner, id) {
	...
	return util
		.paging(this.spotify, this.spotify.getPlaylistTracks, [owner, id])
		.then((response) => {
