...
Queue.prototype.dedup = function () {
	...
	return this.forEachPromise((entry) => {
		...
		return other.getPopularity().then((otherPopularity) =>
			entry.getPopularity().then((entryPopularity) => {
...
Queue.prototype.dispatch = function () {
	return this.forEachPromise((entry) => entry.dispatch());
};
...
Queue.prototype.indexOf = function (obj) {
	return _.findIndex(
		...
		(entry) =>
			entry === obj ||
			(entry?.similarTo && obj && obj.similarTo && entry.similarTo(obj)),
	);
};
