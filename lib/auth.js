...
SpotifyAuth.prototype.implicitGrantFlowURI = function (uri, clientId, scopes) {
	...
	url += `/?client_id=${encodeURIComponent(clientId)}&response_type=token&redirect_uri=${encodeURIComponent(uri)}`;
	if (scopes) {
...
SpotifyAuth.prototype.implicitGrantFlow = function (clientId, scopes, port) {
	return new Promise((resolve, reject) => {
		...
		const login = this.implicitGrantFlowURI(localhost, clientId, scopes);
