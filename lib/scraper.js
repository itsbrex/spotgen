 ...
 */
WebScraper.prototype.pitchfork = (uri, count) => {
	count = count || 0;
	function getPages(nextUri, result, count) {
		nextUri = URI(nextUri).absoluteTo(uri).toString();
		console.log(`${nextUri}\n`);
		return http(nextUri).then((data) => {
			...
			html.find('div[class*="artist-work"]').each(function () {
				const artist = util.normalize(
					$(this).find('ul[class*="artist-list"] li:first').text(),
				);
				const album = util.normalize(
					$(this).find('h2[class*="work-title"]').text(),
				);
 ...
 * @return {Promise | string} A newline-separated list of albums.
 */
WebScraper.prototype.rateyourmusic = (uri, count) => {
	count = count || 0;
	function getPages(nextUri, result, count) {
		nextUri = URI(nextUri).absoluteTo(uri).toString();
		console.log(`${nextUri}\n`);
 ...
 * @return {Promise | string} A newline-separated list of tracks.
 */
WebScraper.prototype.reddit = (uri, count) => {
	count = count || 1;
	function getPages(nextUri, result, count) {
		nextUri = URI(nextUri).absoluteTo(uri).toString();
		console.log(`${nextUri}\n`);
 ...
 * @return {Promise | string} A newline-separated list of tracks.
 */
WebScraper.prototype.webpage = (uri) => {
	console.log(`${uri}\n`);
	return http(uri).then((data) => {
		const html = $($.parseHTML(data));
		let result = "";
 ...
 * @return {Promise | string} A newline-separated list of tracks.
 */
WebScraper.prototype.youtube = (uri) => {
	console.log(`${uri}\n`);
	return http(uri).then((data) => {
		const html = $($.parseHTML(data));
		let result = "";
		html
			.find("div.playlist-video-description h4, a.pl-video-title-link")
			.each(function () {
				const track = util.stripNoise($(this).text());
				result += `${track}\n`;
			});
