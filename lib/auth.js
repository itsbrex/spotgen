 ...
 */
SpotifyAuth.prototype.clientsCredentialsFlow = function (
	clientId,
	clientKey,
	grantType,
) {
	const id = clientId || this.clientId;
	const key = clientKey || this.clientKey;
	const grant = grantType || "client_credentials";
	const auth = `Basic ${base64.encode(`${id}:${key}`)}`;
...
SpotifyAuth.prototype.implicitGrantFlowURI = function (uri, clientId, scopes) {
	...
	url += `/?client_id=${encodeURIComponent(id)}&response_type=token&redirect_uri=${encodeURIComponent(uri)}`;
	if (scopes) {
...
SpotifyAuth.prototype.implicitGrantFlow = function (clientId, scopes, port) {
	return new Promise((resolve, reject) => {
		const id = clientId || this.clientId;
		const portNumber = port || 9000;
		const localhost = `http://localhost:${portNumber}/`;
		const login = this.implicitGrantFlowURI(localhost, id, scopes);
		const server = express();
		const http = server.listen(portNumber);
		server.get("/", (req, res) => {
			const url = URI(req.url);
			if (url.query()) {
				http.close();
				req.connection.ref();
				req.connection.unref();
				this.token = url.search(true).access_token;
				if (!this.token) {
					reject(this.token);
				} else {
					resolve(this.token);
