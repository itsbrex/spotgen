	...
	if (str) {
		...
		while (lines.length > 0) {
			...
			if (
				...
				(match = line.match(/^#(alternate|interleave)\s*BY\s+(.*)/i))
			) {
				...
				collection.add(
					new Artist(this.spotify, match[2], null, Number.parseInt(match[1])),
				);
			} else if ((match = line.match(/^#top([0-9]*)\s+(.*)/i))) {
				collection.add(
					new Top(this.spotify, match[2], null, Number.parseInt(match[1])),
				);
			} else if ((match = line.match(/^#similar([0-9]*)\s+(.*)/i))) {
				collection.add(
					new Similar(this.spotify, match[2], null, Number.parseInt(match[1])),
				);
			} else if (
				(match = line.match(
					/^#playlist([0-9]*)\s+([0-9a-z]+)[\s/:]+([0-9a-z]+)/i,
				))
			) {
				collection.add(
					new Playlist(
				...
				(match = line.match(
					/^([0-9]+ )?https?:\/\/(.*\.)?spotify\.com\/(.*\/)*artist\/(.*\/)*([0-9a-z]+)/i,
				))
			) {
				collection.add(
					new Artist(this.spotify, line, match[5], Number.parseInt(match[1])),
				);
			} else if ((match = line.match(/spotify:album:([0-9a-z]+)/i))) {
				collection.add(new Album(this.spotify, line, null, null, match[1]));
			} else if (
				(match = line.match(
					/^([0-9]+ )?https?:\/\/(.*\.)?spotify\.com\/(.*\/)*album\/(.*\/)*([0-9a-z]+)/i,
				))
			) {
				collection.add(
					new Album(
				...
				(match = line.match(/spotify:user:([0-9a-z]+):playlist:([0-9a-z]+)/i))
			) {
				collection.add(new Playlist(this.spotify, line, match[1], match[2]));
			} else if (
				(match = line.match(
					/^([0-9]+ )?https?:\/\/(.*\.)?spotify\.com\/(.*\/)*user\/([0-9a-z]+)\/playlist\/([0-9a-z]+)/i,
				))
			) {
				collection.add(
					new Playlist(
				...
				collection.add(
					new Track(this.spotify, line, null, null, null, match[1]),
				);
			} else if (
				(match = line.match(
					/^([0-9]+ )?https?:\/\/(.*\.)?spotify\.com\/(.*\/)*([0-9a-z]+)/i,
				))
			) {
				collection.add(
					new Track(this.spotify, line, null, null, null, match[4]),
				);
			} else if ((match = line.match(/^([0-9]+ )?(https?:.*)/i))) {
				collection.add(
					new WebScraper(match[2], Number.parseInt(match[1]), this),
				);
			} else if (
				(match = line.match(/([^\t]*)\t-\t([^\t]*)(\t-\t([^\t]*))?/i))
			) {
				collection.add(
					new Track(this.spotify, line, match[1], match[2], match[4]),
				);
			} else if (line) {
